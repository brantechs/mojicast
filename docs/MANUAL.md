...
OBSのブラウザソースへ貼り付けるURLの一覧です。**要素ごと**に行が並び、それぞれ `コピー` できます。

| 要素 | 出るもの |
|---|---|
| 全部入り | 文字起こし＋翻訳（従来どおり・`http://localhost:8765`） |
| 文字起こしのみ | 翻訳の併記行を出さない（`?part=asr`） |
| 翻訳1〜3 | その翻訳だけを字幕として出す（`?part=tr1`〜`tr3`）。翻訳がオンで翻訳先が設定されている数だけ並びます |

各行の `透過` を外して色を選ぶと、その色で塗ったページのURL（`&bg=RRGGBB`）になります。透過のままなら従来どおり映像に重ねられます。
`接続設定` でポートを変えられます（[4章 OBS接続](#4-アプリ設定声の聞き取り翻訳コラボ接続)）。
...
左メニューには **🛠 スタジオ** と **⚙ アプリ設定** の2つのツリーが常に並び、いま開いている側が
色付きで示されます。スタジオ側の項目を押すと、**同じ窓のまま**スタジオへ切り替わります。

> 以前あった「AIモデル設定窓」と「1対1コラボ設定窓」は、この**アプリ設定に統合**されました。
> さらに**スタジオ・アプリ設定・リリック演出ラボは1つの窓を共有**します。
> 窓は「コックピット」と「この窓」の2つだけで、行き来しても増えません。
...
- `2つ目の翻訳言語（同時に併記）` / `3つ目の翻訳言語（同時に併記）`: 同じ言語リストから選ぶと、確定字幕の下に**最大3言語ぶんの訳文を同時に併記**できます（既定はどちらも「追加しない」＝1言語のみ）
- 選んだ言語それぞれに対して**実際に使われる翻訳モデル**（FuguMT / M2M-100 / ＋OpenCC）が表示されます。先に選んだ翻訳先と同じ言語を選んだ場合は1つにまとめられます
- 言語を増やすほど、ロードされる翻訳モデルとメモリ・処理量が増えます。**低スペック機では1言語のみ**をおすすめします
...
- `表示用URL` … 要素（全部入り／文字起こしのみ／翻訳1〜3）ごとのURLと `コピー`、推奨サイズの案内。
  各行の `透過` を外すと背景色を選べます（[3章 OBSに表示](#obsに表示)・[5章](#5-obs-に字幕を出す)）
...
| AI辞書用ログを書き出す | 直近1か月ぶんの文字起こし＋AI用の手順書をZipにまとめます。押すと書き出し先フォルダが開きます。使い方は[6章](#aiと一緒に辞書をつくる) |
| 画面テーマ | ライト（既定）／ダーク。**OBSに映る字幕の色には影響しません** |
| 開発者ログを残す | 既定オフ。不具合を報告するときだけオンに（[9章](#開発者ログ)）。次の**■停止 → ▶開始**から記録されます |
...
「OBSに表示」の一覧から要素ごとのURLをコピーして、**別々のブラウザソース**に貼り付けます。

| URL | 出るもの |
|---|---|
| `http://localhost:8765` | 全部入り（文字起こし＋翻訳） |
| `http://localhost:8765/?part=asr` | 文字起こしだけ |
| `http://localhost:8765/?part=tr1` | 1つ目の翻訳だけ（`tr2` `tr3` は2・3つ目） |

翻訳だけのソースでは、訳文がふつうの字幕と同じ大きさ・同じ流れ方で出ます（位置や見た目は字幕スタジオの設定に従います）。
好きな位置に置ける代わりに、ソースの数だけOBSの負荷が増えるので、必要なぶんだけ足してください。

背景を塗りたいときは `&bg=RRGGBB`（例: `?part=tr1&bg=101820`）を付けます。「OBSに表示」で `透過` を外して色を選べば、
このURLが自動でできます。付けなければ従来どおり透過です。
...
左メニューの下には **⚙ アプリ設定** のツリーも並んでいます。押すと**同じ窓のまま**アプリ設定へ切り替わり、
戻るときも同じように 🛠 スタジオ の項目を押します（[4章](#4-アプリ設定声の聞き取り翻訳コラボ接続)）。
リリック演出ラボも同じ窓で開き、左上の `← スタジオへ戻る` で戻れます。
...
- 通常表示では `文字揃え` をアイコンで指定できます。**横（左・中央・右）と縦（上寄せ・上下中央・下寄せ）の組み合わせ**で、枠のどこに字幕を寄せるかが決まります
- `1行に収める` をオンにすると、長い文でも折り返さずに文字サイズを縮めて1行に納めます
- 翻訳ONのときは、日本語の確定と同時に訳文の行場所を確保します。あとから訳文が届いても字幕が押し上がりません
...
「どの単語を登録すればいいか分からない」ときは、**実際の配信ログをAIに読ませて
探してもらう**のが近道です。過去の文字起こしをZipにまとめる機能があります。

1. `アプリ設定` → `その他` → **`AI辞書用ログを書き出す`** を押す
   （`字幕ログを保存する`がオンの日ぶんだけが対象。最大で直近1か月ぶん）
2. 開いたフォルダの `dict_logs_日時.zip` を、ChatGPT や Claude などの
   **Web版AIにそのまま添付**する
3. Zipの中の **`AI辞書インタビュー.md`** に依頼文が入っています。その
   「AIへの依頼文」の部分をコピーしてAIに貼り付ける
4. AIが誤認識らしき語を探し、**前後の文脈を見せながら1件ずつ**
   「正しい表記は？」「読みは？」と聞いてくれます
5. 最後にAIが `表記,読み,出やすさ` の形でまとめて出力するので、それを
   `🎤 認識させる単語` の表へ入力する（数が多いときは、Mojicastを終了してから
   `data\hotwords.txt` に直接貼り付けてもOK。次回起動時に反映されます）

> 送られるのは**書き出したZipだけ**です。Mojicastが自動で外部へ送信することはありません。
> 配信内容をAIに渡してよいかは、Zipの中身（テキスト）を確認してから判断してください。
...
- **最大3言語の同時併記**: 「2つ目／3つ目の翻訳言語」を選ぶと、確定字幕の下に最大3言語ぶんの
  訳文が設定順に積まれて表示されます（例: 英語＋繁体字台湾＋韓国語）。必要なモデルはすべて
  ロードされるため、**言語を増やすほどメモリと翻訳の処理量が増えます**。低スペック機では
  1言語のみをおすすめします（訳文の遅れや取りこぼしが増えたら言語を減らしてください）。
  先に選んだ翻訳先と同じ言語を選んだ場合は1つにまとめられます
- リリックビデオ風字幕では、訳文は行の下ではなく**そのフレーズのシーン下部**に併記されます
...
| `logs\daily\` | 同じ文字起こしの**日付ごとのまとめ**（`01.txt`〜`31.txt`・直近1か月ぶん） |
| `logs\日付\*.jsonl` | 「開発者ログを残す」オン時だけの調査用ログ（下記） |
| `data\export\` | 書き出したファイルの置き場。`AI辞書用ログを書き出す`の `dict_logs_日時.zip` と、スタジオの `.mojipack`（[6章](#6-スタジオ字幕の見た目と言葉)）が同じ場所に入ります |
...
**`logs\daily\`（日付ごとのまとめ）**: 上のセッション別ログと**同じ内容**を、日付ごとに
1ファイルへまとめたものです。ファイル名は日番号（`01.txt`〜`31.txt`）で、
**月が変わると同じ日番号のファイルを上書き**するため、常に直近1か月ぶんだけが残ります
（自分で消す必要はありません）。`AI辞書用ログを書き出す`はここを読んでZipにまとめます。

**`data\export\`**: `AI辞書用ログを書き出す`で作ったZip（`dict_logs_20260814_213000.zip` など）の置き場です
（スタジオの`.mojipack`と同じ「書き出し先」フォルダなので、迷ったらここを見てください）。
Zipには日付ごとのログと、AIへの依頼文 `AI辞書インタビュー.md`（書き出しのたびに
`logs\` 直下へ最新版が作られます）が入ります。不要になったZipは削除してOK。
使い方は[6章「AIと一緒に辞書をつくる」](#aiと一緒に辞書をつくる)へ。

### 開発者ログ

「訳文だけがおかしい」「この言い回しのときだけ字幕が変」といった報告を調べるための、
**不具合調査専用のログ**です。アプリ設定の「その他 → 開発者向け → 開発者ログを残す」を
オンにして、**■停止 → ▶開始**しなおすと記録が始まります（既定オフ。字幕の見た目・
動作・速度は変わりません）。

- 保存先は字幕ログと同じ `logs\日付\` で、**同じファイル名の `.jsonl`**
  （例: `2026-08-16_213000.log` と `2026-08-16_213000.jsonl`）
- 「字幕ログを保存する」がオフでも記録されます（逆に、こちらだけオフにもできます）
- 1行1件のJSONで、次の3種類が時刻つきで並びます
  - `session` … そのセッションの認識モデル・翻訳先・句読点などの設定
  - `final` … 確定した1行。`steps` に「認識した生の文 → 単語置換 → 数字 → 句読点 →
    伏せ字」のうち**文が変わった段階だけ**が並ぶので、どの処理が原因かが分かります
  - `translate` … 翻訳の入力（`src`）・出力（`out`）・翻訳先（`lang`）・エンジン（`engine`）・
    所要ミリ秒（`ms`）。英訳辞書で置換した場合は実際に渡した文（`glossed`）も入ります
- 時刻は字幕ログと同じ書式（ミリ秒つき）なので、**両方を並べて突き合わせ**られます
- 発言内容がそのまま入ります。共有する前に中身を確認してください
- 自動削除はされません。調査が済んだらフォルダごと削除してOK

報告のときは、この `.jsonl` と（あれば）同じ名前の `.log` を添えていただけると原因が特定できます。
取り出しは「その他 → 字幕ログの保存フォルダを開く」から。
