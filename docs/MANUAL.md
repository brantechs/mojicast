...
- `2つ目の翻訳言語（同時に併記）`: 同じ言語リストから選ぶと、確定字幕の下に**2言語ぶんの訳文を同時に併記**できます（既定は「なし」＝1言語のみ）
...
- **2言語の同時併記**: 「2つ目の翻訳言語」を選ぶと、確定字幕の下に2言語ぶんの訳文が
  設定順に積まれて表示されます（例: 英語＋繁体字台湾）。必要なモデルは両方ロードされるため、
  メモリと翻訳の処理量が増えます。低スペック機では1言語のみをおすすめします
