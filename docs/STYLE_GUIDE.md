...
- **無音のあと字幕をまとめて消す**: 指定した秒数だけ字幕が動かなかったら、出ている行を**まとめて**フェードアウトします。
  確定字幕・翻訳・話している途中の薄文字が届くたびに数え直すので、しゃべり続けている間は消えません。
  黙ったときだけ画面をすっきりさせたいときに。**-1（既定）で消さない**＝従来どおり残ります
- **前の字幕の扱い**: 「話している間に消える」にすると、**次の発話を話し始めた瞬間**（薄文字が出た瞬間）に
  前の字幕がフェードで消えます。テロップのように**いつも最新の1文だけ**を見せたいときに。
  既定は「残す」＝従来どおり行が積み上がります。**無音のあとの一括消去とは併用でき**、
  「話し始めたら前の行が消える／黙ったら最後の1行も消える」という組み合わせになります
